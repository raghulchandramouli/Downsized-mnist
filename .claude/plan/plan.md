...
study from the paper, organized as runnable, self-contained marimo notebooks,
...
### 3. Experiment notebooks — `ablations/` (marimo `.py`, one per case study)
Replace the empty `Double_Descent.ipynb` stub with `ablations/double_descent.py`
and add the rest of the paper's case studies as siblings:

1. `baseline_classification.py` — train LogisticRegression/MLP/CNN/GRU on
...
2. `tsne_comparison.py` — t-SNE on raw MNIST-1D vectors vs. real MNIST (via
   ...
   does not.
3. `lottery_tickets.py` — iterative magnitude pruning on an MLP (Frankle &
...
4. `double_descent.py` — train MLP/CNN with 15% label noise across a sweep of
...
5. `metalearn_learning_rate.py` — unrolled/differentiable optimization
   ...
   of init.
6. `metalearn_activation_function.py` — parameterize an activation function
...
7. `self_supervised_learning.py` — SimCLR-style contrastive pretraining using
...
8. `benchmark_pooling.py` — train CNNs with each pooling variant
...
matplotlib plots inline as marimo cell outputs.
...
as a clean marimo notebook rather than copy-pasting.
...
- Execute every `ablations/*.py` notebook as a plain script
  (`.venv/bin/python ablations/<file>.py`) and confirm it completes without
